...
## 2026-08-05 — M5: dashboard built without the precision/recall panel `docs/PRD.md` FR13 asks for

**Decision:** `src/monitoring/metrics.py` exposes a Prometheus `/metrics` endpoint on the scoring API with latency (histogram), throughput/decision counts (counter), fraud-probability distribution (histogram), and drift PSI per monitored column (gauge, refreshed from a file `scripts/check_drift.py` writes each run since drift checks aren't a continuously-running process). `docker-compose.yml` now also starts Prometheus + Grafana, with the dashboard (`configs/grafana/provisioning/dashboards/tripwire.json`) auto-provisioned. This covers 3 of `docs/ARCHITECTURE.md` §2.6's 4 listed dashboards: latency/throughput, prediction distribution, drift PSI over time.
...
## 2026-08-06 — Deployment prep: object-storage-backed model fetch + containerized API

User was asked what to build/improve before deployment; the concrete gap was that only the supporting services (Redpanda/Prometheus/Grafana) had a Docker story — the API itself had no `Dockerfile`, and `models/registry/` is gitignored, so a deployed container would boot with nothing to serve. User explicitly chose: fetch the model from object storage at startup, not bake it into the image.

**`boto3` dependency (`uv add boto3`):** an S3-compatible client. Works against AWS S3, Cloudflare R2, MinIO, or anything else that speaks the S3 API via `configs/deployment.yaml`'s `endpoint_url` override — no vendor lock-in to a specific provider's SDK.

**Design (`src/models/artifact_store.py`):** `sync_registry()` downloads `pointers.json` plus whichever production/shadow `.joblib` files it references, called once at FastAPI startup. `publish_registry()` (used by the new `scripts/publish_model.py`) is the deploy-time counterpart — upload after training/promoting locally, then restart the deployed API to pick it up. **Opt-in, not a behavior change**: `configs/deployment.yaml`'s `model_bucket` is empty by default, and `is_enabled()` gates every call — local dev keeps working exactly as it did before this, straight off `models/registry/` on disk. Credentials are never read from `configs/*.yaml` (never committed) — boto3's standard `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` env vars or an IAM role, the same pattern any real deployment already needs.

**`Dockerfile`:** uv-based multi-layer build (dependencies cached separately from app code), no model baked in per the user's explicit instruction, `HEALTHCHECK` against `/v1/health`.

**Two real bugs found and fixed by actually building and running the image, not by inspection:**
1. The first build attempt pulled the full NVIDIA CUDA stack for `torch` (`nvidia-cublas` 517MB, `nvidia-cudnn` 424MB, `triton` 176MB, ~20 CUDA packages, multiple GB total) — PyPI's default Linux wheel for `torch` includes GPU support, and this project never trains/serves on a GPU. Fixed by pinning `torch` to PyTorch's CPU-only wheel index in `pyproject.toml`'s `[tool.uv.sources]`/`[[tool.uv.index]]` — `uv lock` confirmed removing ~20 `nvidia-*`/`triton`/`cuda-*` packages, no change to local dev (macOS was already getting CPU wheels; this only affects what a Linux container resolves).
2. `CMD ["uv", "run", "uvicorn", ...]` silently re-syncs against `pyproject.toml` on every container start — including the dev dependency group (`mypy`, `ruff`, ...) — meaning every container start made a real network call and multi-MB download it didn't need, despite `uv sync --frozen --no-dev` already having run at build time. Fixed by invoking the venv's own `uvicorn` binary directly (`ENV PATH="/app/.venv/bin:$PATH"`) instead of going through `uv run` at runtime.
3. (Also hit, less surprising:) `python:3.11-slim` doesn't ship `libgomp1` — LightGBM's compiled extension needs it (`OSError: libgomp.so.1: cannot open shared object file`), confirmed by an actual failed container start, not assumed. Fixed with a minimal `apt-get install libgomp1`.

**Verified end-to-end, not just "should work":** built the image (`docker build -t tripwire-api .`, ~71s, final size 1.58GB), ran it with a local `models/registry/` volume mount (`docker run ... -v $(pwd)/models/registry:/app/models/registry`), confirmed `/v1/health` returns the real production/shadow versions, `POST /v1/score` returns a real scored decision with production + shadow + sequence-model logs all present, and Docker's own `HEALTHCHECK` reports `healthy` after the container settles.

**Not yet verified (flagged, not silently skipped):** the object-storage fetch path itself (`sync_registry()`) is unit-tested with a mocked S3 client (`tests/unit/test_artifact_store.py`) but never exercised against a real bucket — no AWS/R2 account was available to test against in this session. The local-volume-mount path (verified above) is the one to trust today; treat the object-storage path as built-and-unit-tested, not production-verified, until it's run against a real bucket once.
