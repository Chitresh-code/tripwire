...
Canonical columns: transaction_id, account_id, timestamp, amount, is_fraud.
Everything downstream (src/features/, src/models/) reads this shape, not
PaySim's raw column names — so swapping datasets later only means changing
the loader, not every feature.
"""
...
def load_paysim(path: Path | str) -> pd.DataFrame:
    ...
    raw = pd.read_csv(path, usecols=["step", "nameOrig", "amount", "isFraud"])
